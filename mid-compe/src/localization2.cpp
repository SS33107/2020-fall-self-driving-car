...
class localization{
  ...
  private:
    ...
    int count=1;
    string map_path ="$(find localization_309605004)/map/nuscenes_map.pcd";
    string csv_path ="$(find localization_309605004)/submit_2.csv";
...
localization::localization(ros::NodeHandle nh)
{
  ...
  passMx.setFilterLimits (1600, 2300);
  ...
  passMy.setFilterLimits (750,1000);
...
{ 
  ...
  if(count<250) {
   passCx.setFilterLimits (-40,100);
   passCy.setFilterLimits (-50, 60);
   passCz.setFilterLimits (2.5, 50);

  }
  
  else{
   passCx.setFilterLimits (-35,100);
   passCy.setFilterLimits (-70, 70);
   passCz.setFilterLimits (4, 50);
   }
...
void localization::gps_Callback(const geometry_msgs::PointStamped::ConstPtr& msg)
{
  ...
  gps_z = msg->point.z;
  if(count < 2){
    double theta = 1.5*3.14159/180;
    init_guess <<     cos(theta), -sin(theta),  0.0, 1772.0362265,
